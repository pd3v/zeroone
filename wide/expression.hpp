...
// rotR with with scale param on currying
// this, to work, would required some structural changes in Instrument
/*function<vector<int>(vector<int>)> rotR(vector<int>& notes) {
  return
  [&notes](vector<int> scale)->vector<int>{
    transform(notes.begin(),notes.end(),notes.begin(),[&scale](int note){
      note += 1;
      note %= scale.size();
      return note;
